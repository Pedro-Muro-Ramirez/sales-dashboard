...
function Form() {
  // Access user data and the current session
  const { users, session } = useAuth();

  // Handles form submission, loading state, and errors
  const [error, submitAction, isPending] = useActionState(
    async (previousState, formData) => {
      // Get the selected sales rep name from the form
      const submittedName = formData.get("name");

      // Find the matching user record
      const user = users.find((u) => u.name === submittedName);

      // Build the new sales deal payload
      const newDeal = {
        user_id: user.id,
  ...
  // Find the currently logged-in user's profile
  const currentUser = users.find((user) => user.id === session?.user?.id);

  // Generate dropdown options for sales reps
